...
// 🔥 VANILLA JS FORMATTER FOR LAST ACTIVE (No npm install needed)
...
export default function Home() {
  ...
  const [lastActiveTime, setLastActiveTime] = useState(null); // 🔥 NEW STATE FOR LAST ACTIVE
    ...
    const getExpoToken = () => {
      return new Promise((resolve) => {
        ...
        const interval = setInterval(() => {
          ...
          attempts++;
          if (attempts > 6) { clearInterval(interval); resolve(null); }
        }, 500);
      });
    };
    ...
    if (expoToken) {
      try {
        await fetch("/api/subscribe", {
          ...
          // 🔥 FIX 1: ALWAYS USE STUDENT ID (UID) TO SAVE TOKEN, NEVER NAME
  ...
  useEffect(() => {
    ...
    const handleBlur = () => { if (ignorePanicRef.current) return; handleLogout(); };
  ...
  useEffect(() => {
    ...
    channel.bind("receive_message", async (data) => {
      // 🔥 FIX 2: STRICT MATCHING USING LOGIN ID ONLY
      if (String(data.senderId).trim().toLowerCase() === studentId.trim().toLowerCase()) return;
      
      setLastActiveTime(Date.now()); // Update last active dynamically
      ...
      try {
        ...
        if (text === "SYS_PING_ACTIVE") {
          setIsPeerActive(true); clearTimeout(peerTimeout.current);
          peerTimeout.current = setTimeout(() => setIsPeerActive(false), 5000); return;
        }

        if (text === "SYS_TYPING_ACTIVE") {
          setIsPeerTyping(true); clearTimeout(typingTimeout.current);
          typingTimeout.current = setTimeout(() => setIsPeerTyping(false), 2000); return;
        }
    ...
    channel.bind("message_seen", (data) => {
       setMessages((prev) => prev.map((msg) => { if (msg.sender === "me" && !msg.seenAt) { return { ...msg, seenAt: Date.now() }; } return msg; }));
    });
    ...
    const pingInterval = setInterval(() => {
      ...
      fetch("/api/pusher", {
        ...
        // 🔥 FIX 3: USE STUDENT ID FOR INTERVAL PINGS
  ...
  const dispatchMessage = async (msgText) => {
    ...
    await fetch("/api/messages/send", {
      method: "POST", headers: { "Content-Type": "application/json" },
      // 🔥 FIX 4: STRICT IDENTITY MATCHING FOR DB
      body: JSON.stringify({ id: tempId, encryptedText, senderId: studentId.trim(), receiverId: targetNode, channel: activeChannel, timeStr }),
    });
  };
  ...
  const handlePingPartner = async () => {
    ...
    try {
      await fetch("/api/ping", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // 🔥 FIX 5: ADDED MISSING `receiver` FIELD!
    ...
    <div className={`font-sans flex flex-col overflow-hidden transition-colors duration-300 ${isDarkMode ? "text-slate-200" : "text-slate-800"}`} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: viewportHeight, backgroundColor: isDarkMode ? '#09090b' : '#f4f5f9' }}>
      ...
      <div className={`px-4 py-3 flex justify-between items-center z-20 shadow-sm shrink-0 transition-colors duration-300 border-b ${isDarkMode ? "bg-[#18181b] border-[#27272a]" : "bg-white border-slate-100"}`}>
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-full transition-colors ${isDarkMode ? "bg-[#27272a] text-amber-400 hover:bg-[#3f3f46]" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>{isDarkMode ? <Sun size={18} /> : <Moon size={18} />}</button>
      ...
      <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 z-10 scrollbar-hide relative will-change-scroll w-full" style={{ backgroundImage: isDarkMode ? bgPatternDark : bgPatternLight, backgroundSize: "180px 180px" }}>
        <AnimatePresence>
          {messages.map((m) => {
            const isImage = m.text.startsWith("IMG_SYS::"); const parts = isImage ? m.text.split("::") : []; const imageUrl = isImage ? parts[1] : null; const isMe = m.sender === "me";
            return (
            <motion.div key={m.id} initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.3 } }} className={`flex flex-col w-full ${isMe ? "items-end" : "items-start"} will-change-transform`}>
      ...
      <AnimatePresence>
        {expandedImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 will-change-transform" onClick={() => setExpandedImage(null)}>
            ...
            <motion.img initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }} src={expandedImage} alt="Photo View" className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl will-change-transform pointer-events-none select-none" />
          </motion.div>
        )}
      </AnimatePresence>
